...
export class Guard {
    ...
    public static againstBadCharacters (argument: any, argumentName: string): IGuardResult {
        const invalidCharacters = /[^a-zA-Z0-9\-]/;
        if (invalidCharacters.test(argument)) {
            return { succeeded: false, message: `${argumentName} has invalid characters` }
        } else {
            return { succeeded: true }
        }
    }
